...
class Stats extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $date = date("Y-m-d");
        $year = date("Y");
        $TodaySales = Sale::find_by_sql("select sum(total) AS sum FROM food_sales where created_at = '$date'");
        $Top5product = Sale_item::find_by_sql("select name, product_id, sum(qt) AS totalquantity FROM food_sale_items where DATE_FORMAT(date, '%Y') = $year GROUP BY product_id ORDER BY SUM(qt) DESC LIMIT 5");
