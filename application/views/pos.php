 ...
 <section class="wrapper">
  ...
  <div class="col-sm-12">
   ...
   <div class="card">
    ...
    <h3 class="text-center choose_store"><?=label('ChooseStore');?></h3>    
	</header>	
  ...
  <div class="adv-table">
         ...
         <ul id="storeline">
           ...
           <?php }else{ ?>
            ...
            <a href="javascript:void(0)"  onclick="OpenRegister(<?=$store->status ? $store->status : 0;?>, <?=$store->id;?>, '<?=$this->user->role;?>')">
             <div class="col-md-4">
             <div class="statCart Statcolor">              
                <div class="image_wrapper">
                  <img src="<?=base_url()?>assets/img/store.svg" alt="store" height="100" width="100">
                </div>
                <div class="info">
                 <h2 class="count"> <span class="store_title"><?=$store->name;?></span></h2>
                  <span class="store_info"><?=$store->city;?> <span>&bull;</span> <?=$store->phone;?> <span>&bull;</span> <?=$store->email;?></span>
                </div>
               <h3 class="btn btn-green"><span class="tore_type <?= $store->status == 1 ? 'store_open' : 'store_close';?>"><?= $store->status == 1 ? label('open') : label('close');?></span> </h3> 
   ...
   $(function() {
      $('#cachIH').submit(function(event){
         console.log("aki si llega");
         var CashinHand = $('#CashinHando').val();
         var store = $('#store').val();
         $.ajax({
            // url : " echo site_url('pos/openregister')?>",
            url:"application/views/controller/controllerStore.php?source=post", 
            type: "POST",
            dataType:"JSON",
         data: {"cash": CashinHand, "store": store, "waitersCach": waitersCach,"id_user":<?php echo $this->session->userdata('user_id');?>},
             success: function(data)
             {
             ...
             error: function (jqXHR, textStatus, errorThrown)
             { swal('La caja ha sido abierta',"","success",{timer:3000,buttons:false});

               window.location.href = "http://10.0.0.7/Food3";
             }
         });
...
<div class="panel-body">
         ...
         <a class="btn btn-danger float-right" style="margin-top:60px" href="pos/selectTable/0">Realizar una venta a <?=label("WalkinCustomer");?></a>
         <?=!$zones ? '<h4 style="margin-top:60px">'.label("NoTables").'</h4>' : '';?>
         <?php foreach ($zones as $zone):?>

        <div class="row">
            <h3><span class="btn btn-green"> <?=$zone->name;?></span> </h3><hr>
         </div>
...
<div class="container-fluid">
   ...
   <div class="row">
     <ul class="cbp-vimenu2">
        ...
        <li class="cocina2" data-toggle="tooltip"  data-html="true" data-placement="left" title="<?=label('Kitchenpage');?>"><a href="kitchens"><i class="fa fa-cutlery fa-2x" aria-hidden="true"></i></a></li>
      </ul>
...
<div class="modal fade" id="AddSale" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
 <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h3 style="font-family: 'Raleway-Medium';"> <?=$table->name;?> (<?=$header;?>)</h3>
